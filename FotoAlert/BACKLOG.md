...
1. 🔴 **Wohin sollen die Beispielbilder gesichert werden?** Die Metadaten (welches Bild gehört zu welcher Location) landen aktuell im privaten Git-Backup-Repo (`FOTOALERT_BACKUP_REPO`, wird bei jedem Bild-Upload/-Löschen sowieso schon mit angestoßen). Bilddateien sind aber binäre JPEGs — Git-Repos werden mit vielen Bildversionen im Lauf der Zeit spürbar größer und langsamer beim Klonen. Sollen die Bilder trotzdem ins bestehende Git-Backup-Repo (gemeinsam mit den JSON-Dateien), oder in einen separaten lokalen Snapshot-Ordner analog zu den DB-Snapshots (7 Versionen, kein Git)? Die Empfehlung unten (Option A) schlägt das Git-Repo vor, das ist aber eine funktionale Weichenstellung, die vor Umsetzung bestätigt werden sollte.
2. 🔴 **Was passiert mit gelöschten Bildern in alten Backups?** Wenn ein Host ein Bild löscht oder ersetzt: Soll die alte Bilddatei auch aus dem Backup entfernt werden (Backup folgt exakt dem aktuellen Stand), oder bleiben alte Bilder in vergangenen Backup-Versionen erhalten (mehr Sicherheit gegen versehentliches Löschen, aber wachsender Speicherbedarf über Zeit)? Das betrifft die Restore-Erwartung: „stellt das letzte Backup exakt den aktuellen Stand wieder her" vs. „das Backup ist auch ein Sicherheitsnetz gegen eigene Fehlbedienung".
3. ⚪ Annahme: Backup passiert **sofort bei jedem Bild-Upload/-Löschen** (huckepack auf dem bereits bestehenden `backup_after_edit()`-Aufruf), nicht periodisch/zeitgesteuert. Default folgt dem bereits etablierten Muster für DB-Metadaten. — bitte bestätigen.
4. ⚪ Annahme: Es wird eine **Vollkopie** aller aktuell vorhandenen Bilder gesichert (kein inkrementelles Diffing wie rsync), weil die Bildmenge bei „ein Bild pro Location" überschaubar bleibt (aktuell in der Sandbox-Kopie 50 Dateien / 612 KB gesamt, allerdings inkl. mehrerer Test-Artefakte aus US-120/US-125/US-126 — der reale Live-Bestand kann kleiner oder anders zusammengesetzt sein und wurde hier nicht geprüft). — bitte bestätigen.
5. ⚪ Annahme: Bei einer Race Condition (Backup läuft genau während ein neuer Upload die Datei ersetzt) wird in Kauf genommen, dass im ungünstigsten Fall die alte oder eine unvollständige Version im Backup landet — kein Datei-Lock, da Uploads laut Code bereits atomar (`tmp` + `replace()`) geschrieben werden und ein doppelter Verlust (Server-Absturz genau in diesem Sekundenfenster) extrem unwahrscheinlich ist. — bitte bestätigen.

**Example Mapping**

- **Rule 1:** Wird ein Beispielbild hochgeladen oder ersetzt, landet es beim nächsten Backup-Lauf mit im Sicherungsort.
  - Beispiel: Ein Host lädt für eine Location ein neues Titelbild hoch. Kurz danach läuft die Sicherung automatisch mit — das neue Bild ist ab diesem Zeitpunkt Teil des Backups.
- **Rule 2:** Wird ein Beispielbild gelöscht, verschwindet es auch aus dem aktuellen Sicherungsstand (Klärungspunkt: ob auch aus älteren Ständen, siehe offene Frage 2).
  - Beispiel: Ein Host entfernt das Beispielbild einer Location. Die Sicherung, die danach läuft, enthält für diese Location kein Bild mehr.
- **Rule 3:** Fällt der Server komplett aus, lassen sich alle zuletzt gesicherten Beispielbilder wiederherstellen — nicht nur die Ortsdaten.
  - Beispiel: Der Server fällt aus und wird neu aufgesetzt. Nach der Wiederherstellung zeigen die Locations wieder ihre zuletzt hochgeladenen Beispielbilder, nicht nur Name/Koordinaten.
- **Rule 4:** Die Sicherung der Bilder darf den laufenden Betrieb nicht spürbar verlangsamen oder blockieren.
  - Beispiel: Während ein Host ein Bild hochlädt, läuft im Hintergrund eine Sicherung — der Host merkt davon nichts, der Upload dauert nicht länger als sonst.

**Akzeptanzkriterien**

1. Nach dem Hochladen eines neuen Beispielbilds ist dieses Bild spätestens mit der nächsten automatischen Sicherung an einem vom Server getrennten Sicherungsort vorhanden.
2. Nach dem Ersetzen eines bestehenden Beispielbilds enthält die nächste Sicherung die neue Version; ob die alte Version aus vergangenen Sicherungen verschwindet oder dort erhalten bleibt, richtet sich nach der Klärung zu offener Frage 2.
3. Nach dem Löschen eines Beispielbilds (eigenständig, ohne die Location selbst zu löschen) verschwindet das Bild aus dem aktuellen Sicherungsstand.
4. Beim Löschen einer ganzen Location verschwindet auch deren Beispielbild aus dem aktuellen Sicherungsstand.
5. Das Hoch-/Runterladen bzw. Löschen eines Bildes fühlt sich für den Host genauso schnell an wie vor dieser Änderung — die Sicherung passiert unbemerkt im Hintergrund.
6. Ist der Sicherungsort (z. B. das Backup-Repo) nicht erreichbar oder nicht eingerichtet, schlägt der normale Bild-Upload/das Löschen trotzdem nicht fehl — die App bleibt für den Host benutzbar, nur die Sicherung selbst wird übersprungen (mit Log-Hinweis).
7. Im Entwicklungs-/Testbetrieb (nicht auf dem echten Server) passiert keine Sicherung von Bildern, genau wie beim bisherigen Verhalten für die Ortsdaten.

**Pre-Mortem**

1. **Speicherplatz bei Bild-Backups:** Da jedes hochgeladene Bild serverseitig auf ca. 500 KB komprimiert wird, wächst der Sicherungsort mit jeder Location um diese Größenordnung. Bei einer Ablage im Git-Repo (Option A) summieren sich außerdem alte Versionen im Git-Verlauf, wenn Bilder wiederholt ersetzt werden — das Repo wächst dauerhaft, auch wenn die aktuelle Bildmenge klein bleibt. Gegenmaßnahme/Klärung: siehe offene Frage 1 und 2.
2. **Backup-Größe/-Dauer:** Solange die Bildmenge klein bleibt (aktuell Größenordnung < 1 MB in der Sandbox-Kopie, echter Live-Bestand ungeprüft), ist die zusätzliche Sicherungszeit vernachlässigbar. Bei deutlichem Wachstum (viele Locations, viele Ersetzungen) könnte ein Git-`add .` spürbar langsamer werden als heute nur mit den beiden JSON-Dateien — sollte beobachtet werden, kein Blocker für den Start.
3. **Restore-Prozess:** Es muss klar dokumentiert sein, wie im Ernstfall vorgegangen wird (welcher Ordner/welches Repo wird auf den neuen Server zurückkopiert, in welcher Reihenfolge relativ zum DB-Restore). Fehlt diese Doku, hilft das beste Backup im Ernstfall nicht, weil unklar ist, wie es zurückgespielt wird.
4. **Konsistenz zwischen DB-Snapshot-Zeitpunkt und Bild-Snapshot-Zeitpunkt:** Die Ortsdaten-Sicherung (bei jedem Edit) und eine mögliche periodische Bild-Sicherung könnten zeitlich auseinanderlaufen, wenn dafür unterschiedliche Auslöser gewählt würden. Dadurch könnte nach einem Restore eine Location auf ein Bild verweisen, das im Bild-Backup noch nicht enthalten ist (oder umgekehrt). Gegenmaßnahme: Bild-Sicherung an denselben Auslöser koppeln wie die JSON-Sicherung (Upload/Löschen-Ereignis), nicht an einen eigenen Zeitplan — das vermeidet dieses Szenario strukturell (siehe Empfehlung Option A).
5. **Race Condition bei gleichzeitigem Upload während Backup:** Läuft die Bildsicherung genau in dem Sekundenbruchteil, in dem eine Datei per Rename ersetzt wird, könnte der Sicherungslauf entweder die alte oder schon die neue Version erwischen — beides ist ein gültiger, in sich konsistenter Zustand (kein halb geschriebenes Bild, da Uploads bereits atomar sind). Kein zusätzlicher Lock-Mechanismus notwendig, siehe Annahme 5 oben.

**Architektur-Analyse — betroffene Dateien**

- `backend/data/backup.py` — Kernänderung: neue Funktion zum Sichern des Bildordners, Erweiterung/Wiederverwendung des bestehenden Git-Commit-Mechanismus bzw. eines neuen Snapshot-Mechanismus.
- `backend/main.py` — Aufrufstellen bei `upload_location_image` (Zeile ~2390) und `delete_location_image` (Zeile ~2428) sowie beim Location-Löschen (Zeile ~2588, ruft bereits `_delete_location_image_file()` auf) nutzen bereits `backup.backup_after_edit(loc_id)` im Hintergrund — hier ggf. um den Bildordner ergänzen, falls Option A gewählt wird.
- `backend/precompute.py` — nur falls Option B (separater Snapshot-Mechanismus analog zu `snapshot_before_precompute()`) gewählt wird; aktuell ruft dort niemand einen Bild-Snapshot auf.
- Keine Frontend-Dateien betroffen (rein serverseitige Infrastruktur, keine sichtbare UI-Änderung für Nutzer).

**Designer-Check:** Übersprungen — TASK-55 ist eine reine Backend-/Infrastrukturänderung ohne jede sichtbare Auswirkung auf Bedienung, Layout oder Optik der App. Es entsteht kein neues UI-Element und keine bestehende Ansicht ändert sich.

**Implementierungsoptionen**

- **Option A — Bilder huckepack im bestehenden Git-Backup-Zyklus (empfohlen):**
  Vorgehen: Die Funktion, die heute schon bei jedem Bild-Upload/-Löschen die Ortsdaten als JSON ins Backup-Repo schreibt und committet, wird so erweitert, dass sie zusätzlich den kompletten aktuellen Bildordner in einen Unterordner des gleichen Backup-Repos kopiert, bevor committet und gepusht wird.
  Betroffene Dateien: `backend/data/backup.py` (Kernlogik), `backend/main.py` (keine Änderung an den Aufrufstellen nötig, da der bestehende Aufruf wiederverwendet wird).
  Vorteile: Nutzt exakt denselben, bereits bewährten Auslöser wie die Ortsdaten-Sicherung — Bild und Datenbank-Verweis bleiben dadurch strukturell zeitlich synchron (löst Pre-Mortem-Punkt 4 sauber). Kein neuer Cronjob/Zeitplan nötig. Restore ist einfach: ein Repo, ein Vorgang.
  Nachteile/Risiken: Git-Repo wächst dauerhaft mit jeder Bildversion (Pre-Mortem 1), da Git von Natur aus jede committete Version dauerhaft in der Historie behält, auch wenn die Datei später ersetzt wird — außer es wird zusätzlich eine Historie-Bereinigung eingeführt (aufwändiger, hier nicht vorgesehen). Etwas höhere Commit-Dauer bei größerer Bildmenge (Pre-Mortem 2).
  Aufwand: Klein bis mittel.

- **Option B — Eigenständiger periodischer Snapshot-Mechanismus für Bilder (analog zu den DB-Snapshots):**
  Vorgehen: Neue Funktion, die – wie heute schon bei den Datenbank-Snapshots vor jedem Precompute-Lauf – den kompletten Bildordner in einen lokalen, rotierenden Snapshot-Ordner kopiert (z. B. letzte 7 Stände), unabhängig vom Git-Backup-Repo.
  Betroffene Dateien: `backend/data/backup.py` (neue Funktion), `backend/precompute.py` (Aufruf an der Stelle, an der heute schon `snapshot_before_precompute()` aufgerufen wird).
  Vorteile: Kein Git-Repo-Wachstum, da lokale Snapshots automatisch rotieren und alte Stände gelöscht werden (bestehendes, bewährtes Muster). Geringeres Risiko für sehr große Repos über Zeit.
  Nachteile/Risiken: Sicherungszeitpunkt ist an den Precompute-Lauf gekoppelt (typischerweise nicht bei jedem Bild-Upload), dadurch potenzielle zeitliche Lücke zwischen einem frischen Bild-Upload und dessen nächster Sicherung — widerspricht dem Wunsch aus der User Story, Bilder unmittelbar gegen Verlust abzusichern (Pre-Mortem 4 wird hier nicht strukturell gelöst, sondern bewusst in Kauf genommen). Snapshots liegen lokal auf demselben Server wie das Original — bei einem kompletten Festplattenausfall (nicht nur Software-Absturz) sind auch die Snapshots weg, anders als beim Git-Repo, das laut TASK-18 typischerweise extern liegt.
  Aufwand: Mittel.

- **Option C — Inkrementelle Spiegelung (rsync) auf einen separaten Sicherungsort:**
  Vorgehen: Ein rsync-artiger Abgleich (nur geänderte/neue Dateien werden übertragen) auf ein separates Ziel (z. B. externer Speicher oder ein weiteres Verzeichnis), ausgelöst bei jedem Bild-Upload/-Löschen oder periodisch.
  Betroffene Dateien: `backend/data/backup.py` (neue Logik), ggf. neue Konfiguration für das rsync-Ziel.
  Vorteile: Effizient bei großen Bildmengen, da nur Änderungen übertragen werden; kein Git-Historie-Wachstum.
  Nachteile/Risiken: Deutlich höherer Umsetzungsaufwand (rsync-Integration, Fehlerbehandlung bei Netzwerkproblemen) für eine Bildmenge, die aktuell klein ist (ein Bild pro Location) — Aufwand steht in keinem Verhältnis zum Nutzen beim aktuellen Datenvolumen.
  Aufwand: Groß.

**Empfehlung:** Option A. Sie nutzt den bereits etablierten und bewährten Backup-Auslöser (gleiche Stelle, die heute schon bei jedem Bild-Upload/-Löschen die Ortsdaten sichert), löst damit strukturell das Konsistenz-Risiko aus Pre-Mortem 4, und der Umsetzungsaufwand ist überschaubar. Das Git-Repo-Wachstum (Nachteil) ist bei der aktuellen kleinen Bildmenge (ein Bild pro Location, ca. 500 KB) noch kein akutes Problem und kann bei Bedarf später durch eine separate Bereinigung adressiert werden, statt die Umsetzung jetzt zu verkomplizieren. Voraussetzung: Klärung der offenen Fragen 1 und 2 vor Implementierungsstart.

**Testplan**

*Automatisiert (pytest, Vorschlag für Testdatei `backend/tests/test_backup.py` oder Erweiterung einer bestehenden Backup-Testdatei falls vorhanden):*

- `test_task55_image_upload_triggers_image_backup()` — Docstring: "TASK-55: Nach Upload eines Beispielbilds enthält der Sicherungsort (Backup-Repo-Unterordner) die neue Bilddatei." Vorgehen: Bild hochladen (Testumgebung mit `FOTOALERT_ENV=prod` und Test-Backup-Repo), danach prüfen ob die Bilddatei im erwarteten Sicherungspfad liegt.
- `test_task55_image_delete_removes_from_current_backup()` — Docstring: "TASK-55: Nach Löschen eines Beispielbilds ist es im aktuellen Sicherungsstand nicht mehr vorhanden." Abhängig von Klärung offene Frage 2.
- `test_task55_backup_noop_in_dev_mode()` — Docstring: "TASK-55: Im Dev-Modus (FOTOALERT_ENV != prod) passiert keine Bild-Sicherung, analog zum bestehenden Verhalten für Ortsdaten." Vorgehen: Funktion mit Dev-Env aufrufen, prüfen dass keine Datei-Operationen stattfinden.
- `test_task55_missing_backup_repo_does_not_break_upload()` — Docstring: "TASK-55: Ist das Backup-Repo nicht eingerichtet, schlägt der Bild-Upload selbst trotzdem nicht fehl (nur die Sicherung wird übersprungen, mit Log-Warnung)." Deckt Akzeptanzkriterium 6 ab.

*Manuell (Browser/curl gegen lokalen Dev-Server, Terminal-Fenster-Modell beachten):*

- Ein Beispielbild für eine Test-Location hochladen, danach im konfigurierten Backup-Repo-Ordner nachsehen, ob die Bilddatei tatsächlich dort angekommen ist.
- Das Beispielbild wieder löschen, danach im Backup-Repo-Ordner prüfen, ob es dort (je nach Klärung offener Frage 2) ebenfalls entfernt wurde oder als alte Version erhalten blieb.
- Prüfen, dass die Antwortzeit des Upload-Endpunkts sich gefühlt nicht spürbar verändert (Sicherung läuft im Hintergrund, blockiert die Antwort nicht).

#### Implementierungsnotiz (TASK-55)

Was das System jetzt tut: Wenn ein Host ein Beispielbild hochlädt, ersetzt, löscht oder eine ganze Location löscht, läuft im Hintergrund dieselbe Sicherung wie schon bisher für die Ortsdaten — nur dass sie jetzt zusätzlich den kompletten Bilderordner mitnimmt. Dabei wird der Bilderordner im Sicherungsort exakt auf den aktuellen Stand gebracht: neue oder ersetzte Bilder werden hineinkopiert, und Bilder, die es auf dem Server nicht mehr gibt (weil sie gelöscht oder ersetzt wurden), werden auch aus dem Sicherungsort entfernt. Es gibt also keinen "Papierkorb" mit alten Bildversionen im Backup — der Sicherungsstand spiegelt immer exakt das, was gerade live auf dem Server liegt. Für den Host ändert sich beim Hochladen/Löschen nichts Spürbares: Die Sicherung läuft weiterhin unbemerkt im Hintergrund, die Antwortzeit bleibt gleich. Ist der Sicherungsort gerade nicht erreichbar oder nicht eingerichtet, wird nur die Sicherung übersprungen (mit Log-Hinweis) — der eigentliche Upload/das Löschen funktioniert trotzdem ganz normal weiter. Im lokalen Testbetrieb passiert wie bisher gar keine Sicherung.

**Technisch:** Geänderte Datei: `backend/data/backup.py`. Neue private Funktion `_sync_image_dir_to_repo()` gleicht `backend/data/location_images/` (Live-Ordner) mit einem neuen Unterordner `location_images/` im Backup-Repo ab — kopiert vorhandene Dateien hinein, löscht dort Dateien, die live nicht mehr existieren. Wird von `backup_after_edit()` direkt vor `_git_commit_and_push()` aufgerufen (huckepack auf dem bereits bestehenden Aufruf, keine Änderung an den Aufrufstellen in `backend/main.py` nötig — Upload/Delete-Image/Image-Focus/Location-Delete lösen `backup_after_edit()` bereits aus). Fehlerbehandlung analog zu `_export_to_repo()`: Exception wird geloggt, gibt `False` zurück, bricht den Request nicht ab. Keine 3.10+-Syntax verwendet (Python-3.9-kompatibel, `py_compile`-Check lokal grün).

#### Testreport (TASK-55)

Neue Testdatei `backend/tests/test_task55_image_backup.py` mit 7 automatisierten Tests, die die Sync-Logik direkt (mit temporären Ordnern statt einem echten Backup-Repo) durchspielen — alle 7 liefen erfolgreich durch. Zur Regressionsprüfung lief zusätzlich die komplette bestehende Test-Sammlung mit; keine bestehende Funktion wurde durch die Änderung beeinträchtigt (die dort aufgetretenen Probleme sind reine Sandbox-Einschränkungen ohne Bezug zu TASK-55, z. B. fehlende Astronomie-Bibliothek — die betroffenen Tests werden dort ohnehin übersprungen, nicht durch diese Änderung ausgelöst).

Pro Akzeptanzkriterium:

1. **Neues Bild landet in der Sicherung** — bestätigt durch Test.
2. **Ersetztes Bild ist in der nächsten Sicherung mit der neuen Version enthalten** — bestätigt durch Test (alte Version verschwindet, wie in der Weg-Gate-Entscheidung festgelegt).
3. **Einzeln gelöschtes Bild verschwindet aus der aktuellen Sicherung** — bestätigt durch Test.
4. **Bild einer komplett gelöschten Location verschwindet ebenfalls** — bestätigt durch Test (die Sync-Logik behandelt beide Lösch-Wege gleich, da für sie nur zählt, welche Dateien noch im Live-Ordner liegen).
5. **Hochladen/Löschen fühlt sich für den Host genauso schnell an wie vorher** — noch zu bestätigen: das lässt sich nur mit echtem Server-Gefühl beurteilen, nicht per automatisiertem Test. Stephan müsste dafür auf dem Produktivserver (oder später beim nächsten echten Rollout) einmal ein Bild hochladen/löschen und darauf achten, ob es sich spürbar langsamer anfühlt als vor dieser Änderung.
6. **Nicht erreichbarer/nicht eingerichteter Sicherungsort lässt Upload/Löschen trotzdem funktionieren** — teilweise bestätigt durch Test: die Sync-Funktion selbst wirft bei einem simulierten Fehler nachweislich keine Exception. Der komplette Weg über den echten Upload-Endpunkt mit einem tatsächlich fehlenden Backup-Repo ist in der Sandbox nicht nachstellbar (dafür bräuchte es `FOTOALERT_ENV=prod`, was den ganzen Backup-Mechanismus erst aktiviert) — das wäre eine zusätzliche manuelle Bestätigung auf dem Server, ist aber aus der Code-Lesung heraus mit hoher Sicherheit unauffällig.
7. **Im lokalen Testbetrieb passiert keine Bild-Sicherung** — bestätigt durch Test.

**Was nur auf dem Produktivserver bestätigt werden kann:** Das Backup-Repo (`FOTOALERT_BACKUP_REPO`) existiert ausschließlich auf dem Server — in der Sandbox ist der komplette Backup-Mechanismus deaktiviert (Entwicklungsmodus). Der volle Weg „Bild hochladen → echtes Git-Commit im Backup-Repo → Bild taucht im Repo-Unterordner `location_images/` auf" lässt sich deshalb nur nach einem Release auf dem Server selbst nachvollziehen, z. B. indem Stephan nach einem Bild-Upload im Backup-Repo-Ordner auf dem Server nachsieht, ob die Datei tatsächlich dort ankommt und der Git-Commit sie enthält.

#### Code-Qualitäts-Check (TASK-55, 2026-07-06)

`tools/refactor_check.py --report` gegen den Gesamtcode ausgeführt: keine Auto-Fixes, 3 bestehende „lange Funktion"-Funde in anderen Dateien (`main.py::startup`, `precompute.py::_run_single_location_flow`, `index.html::mkCloudCompassSvg`) — alle bereits mit eigenen Tickets erfasst (TASK-51, TASK-58 und ein weiteres bestehendes Ticket), keiner davon betrifft `backup.py` oder die neue Testdatei. Für TASK-55 selbst: **sauber, kein Fund.**

Manuelle Durchsicht von `_sync_image_dir_to_repo()` im Vergleich zu den Nachbarfunktionen `_export_to_repo()` und `snapshot_before_precompute()`: Stil, Docstring-Format, Fehlerbehandlung (try/except mit `logger.error`, gibt `bool` zurück, wirft nichts nach außen) und Einbindung in `backup_after_edit()` passen sauber zum bestehenden Muster. Keine Redundanz, keine Lesbarkeitsprobleme, keine Regression. Keine Code-Änderung an `backup.py` nötig gewesen.

**Restore-Doku-Lücke (Pre-Mortem-Punkt 3) geschlossen:** `deploy/restore.sh` kannte `location_images/` bisher gar nicht — es hat bisher nur die beiden JSON-Dateien zurückgespielt und die SQLite-Migration angestoßen, aber keinen Schritt für den Bildordner. Ergänzt: Nach dem JSON-Restore kopiert das Script jetzt zusätzlich `location_images/` aus dem Backup-Repo zurück nach `backend/data/location_images/` (nur wenn der Ordner im Backup-Repo existiert, sonst Warnhinweis statt Absturz). Header-Kommentar des Scripts entsprechend aktualisiert. Das schließt die im Pre-Mortem benannte Lücke „unklar, wie Bilder im Ernstfall zurückgespielt werden" — der Restore-Ablauf ist jetzt: JSON/DB zuerst, danach Bildordner, dann Service-Neustart (Reihenfolge zwischen JSON- und Bild-Restore ist unkritisch, beide sind unabhängig voneinander).
...
### US-21 · App-Beschreibung, Onboarding & verständliche Erklärungen für jede Funktion `[x]`
...
- [x] Beim allerersten App-Start erscheint eine kurze Einführung (3–4 Screens), die erklärt, wofür FotoAlert grundsätzlich da ist und wie die wichtigsten Bereiche (Feed, Kalender, Karte, Locations) funktionieren.
- [x] Ein „?"-Button im Header ist jederzeit erreichbar und öffnet eine Übersicht mit Erklärungen zu Score-System, Schwierigkeitsgraden und Event-Typen (Glossar-Charakter, zentrale Nachschlagestelle).
- [x] Jeder Score-Wert (Astronomie, Wetter, Gesamt, Wolkenstimmung) bleibt wie in US-55 direkt per ⓘ-Button erklärbar — kein Rückbau.
- [x] Zusätzlich zu Scores: **jedes weitere zentrale UI-Element mit eigener Bedeutung** (z. B. Schwierigkeitsgrad-Badge, Event-Typ-Icon, Verifikations-Status, Filter-Chips, Kartensymbole) hat eine eigene, direkt an der Stelle abrufbare Erklärung — nicht nur im zentralen Glossar nachschlagbar.
- [x] Die Erklärung ist ohne Verzögerung erreichbar: höchstens ein Antippen/Klick von der jeweiligen Funktion entfernt, keine Navigation durch mehrere Menüs nötig.
- [x] Onboarding-Screens erscheinen nur beim ersten Start; wiederkehrende Nutzer werden nicht erneut damit konfrontiert, können sie aber über den „?"-Button jederzeit erneut aufrufen.
- [x] Edge Case: Auf kleinen Bildschirmen (iPhone SE-Größe) verdeckt keine Erklärungs-Anzeige (Tooltip/Overlay) dauerhaft wichtige Inhalte darunter — sie lässt sich leicht wieder schließen.
- [x] Edge Case: Wird eine erklärte Funktion später fachlich geändert (z. B. Score-Formel, neuer Event-Typ), ist im Entwicklungsprozess verankert, dass der zugehörige Erklärungstext mit aktualisiert wird (kein „vergessenes" veraltetes Erklär-Overlay).
...
- [x] **Kartenlegende erweitert** um die bisher nicht erklärten Bedienelemente: Kartenebenen-Umschalter (Nacht/Standard/Satellit), GPS-Standort-Button, Klickverhalten der Standort-Symbole auf der Karte (Popup mit Detail-Link), sowie ein Verweis auf die separate Wetter-Legende (US-72).
- [x] **Neuer Glossar-Eintrag „Wie unterscheiden sich Feed, Kalender und Scout?"** erklärt die drei Ansichten anhand ihres tatsächlichen Zeithorizonts (siehe Erklärtexte unten, verifiziert im Code — nicht die ursprüngliche Vermutung).
- [x] **Neuer Glossar-Eintrag „Wie funktionieren die Filter?"** erklärt anhand eines konkreten App-Beispiels, dass Auswahlen innerhalb einer Filtergruppe sich addieren (ODER), Filter über verschiedene Gruppen hinweg sich kombinieren (UND), und dass es bei manchen Filtern (z. B. Verifikation) einen dritten „Ausschließen"-Zustand gibt.
...
**Testplan:**
- [x] Automatisiert (Harness): Prüfbar sind vor allem strukturelle Aspekte (z. B. „Onboarding-Flag wird nach erstem Durchlauf gesetzt und bleibt nach Reload bestehen") — als pytest-/Frontend-Test in der Implementierungsphase konkretisieren.
- [x] Manuell: Neuinstallation/erster Start prüfen (Onboarding erscheint), zweiter Start prüfen (Onboarding erscheint nicht mehr), „?"-Button im Header prüfen (Glossar öffnet), je ein Beispiel-ⓘ bei einem Score (US-55-Bestand) und einem neuen Element (z. B. Schwierigkeitsgrad) prüfen, Test auf kleiner Bildschirmgröße (iPhone SE) gegen Verdeckung.
...
- `web/index.html`: bestätigt vorhandene Infrastruktur — „Sichtachsen-Profil"-Box im Preview-Sheet (Azimut/Distanz/Höhenwinkel, reine Anzeige, **Zeilenreferenz aktualisiert nach US-21, 2026-07-06: jetzt Z. 6609**), Kompass mit Sichtachsen-Pfeil (`mkCloudCompassSvg(sunAz, subjectAz, isRedSky, distanceM)`, **jetzt Z. 3657**, aus US-111; Signatur unverändert, noch kein Status-Parameter — muss bei Implementierung ergänzt werden), gestrichelte Sichtachsen-Linie auf der Karte (`_drawPinsAndLine()`, **jetzt Z. 4009**, Struktur/Parameter unverändert, `dashArray:'6,4'` vorhanden). Zusätzlich existiert bereits die manuelle Meldeoption „Sichtachse blockiert" (**jetzt Z. 2393**) als Nutzer-Feedback-Kanal — aber kein automatischer Check. Die Visualisierungs-Bausteine für ein „frei/blockiert"-Badge sind vorhanden, es fehlt nur der Daten-Wert dahinter.
...
- [ ] **(korrigiert, 2026-07-06, nach Prototyp-Review 2026-07-05)** Der Sichtachsen-Status wird als vierfarbige Status-Pille dargestellt, aber NICHT mit wechselndem Zustands-Icon: Die Pille zeigt ein KONSTANTES Augen-Symbol (unabhängig vom Status immer gleiches Icon) + farbig eingefärbten Text (Frei=Grün `--green`, Teilweise verdeckt=Orange `--orange`, Blockiert=Rot `--red`, Nicht geprüft=Grau `--muted`) — sichtbar sowohl auf der Feed-Karte (`oppCard()`) als auch im Detail-Sheet der Chance; die Pille folgt optisch dem bestehenden Muster der `tag-verified`/`tag-issue`-Tags (kein neues Farbsystem). Der SEPARATE Filter-Chip „Sichtachse" ist von dieser Korrektur NICHT betroffen und behält weiterhin sein eigenes, je nach Filter-Zustand wechselndes Icon (unverändert).
...
- [ ] **(neu, 2026-07-06, Konsistenz-Anforderung nach US-21-Hausstandard)** Sowohl die neue Sichtachsen-Pille als auch der neue Filter-Chip „Sichtachse" bekommen jeweils einen eigenen ⓘ-Info-Button mit eigenem Erklärtext in `ElementInfo`, analog zum Vorbild des Verifikationsfilters (`fInfoBtn('verifikationsstatus')`, Zeile 3556, Erklärtext in `ElementInfo` Zeile 6949-6952) — damit gilt der seit US-21 etablierte Hausstandard „jedes zentrale UI-Element mit eigener Bedeutung hat eine eigene, direkt abrufbare Erklärung" auch für diese beiden neuen Elemente.
...
- `web/index.html` — „Sichtachsen-Profil"-Box (Zeilenreferenz aktualisiert nach US-21, 2026-07-06: jetzt Z. 6609) um Statusanzeige (frei/teilweise/blockiert/nicht geprüft) ergänzen; keine neue Sektion nötig, Erweiterung der bestehenden. Zusätzlich: neuer Menüpunkt im Refresh-Menü (siehe Baustein unten). Event-Detail-Sheets für Himmelsröte-Chancen zeigen künftig ebenfalls den Sichtachsen-Status (nicht nur Alignment-Event-Sheets).
...
**1. Status-Pille (vier Zustände) — KORRIGIERT nach Prototyp-Review, von Stephan final freigegeben am 2026-07-05**
- **Wichtige Korrektur (2026-07-05, ersetzt die ursprüngliche Designer-Entscheidung unten):** Die Pille zeigt KEIN wechselndes Zustands-Icon (Häkchen/Warndreieck/Verbotsschild/Fragezeichen) mehr. Stattdessen: ein KONSTANTES Augen-Symbol (unabhängig vom Status immer dasselbe Icon) + farbig eingefärbter Text. Farben: Frei = Grün `--green`, Teilweise verdeckt = Orange `--orange`, Blockiert = Rot `--red`, Nicht geprüft = Grau `--muted`. Das neue Augen-Icon kommt als neues Symbol in den `<defs>`-Block: geometrisch, Kreis mit liegendem Oval/Mandel-Form (wie ein Auge), 2px Strich, gestalterisch analog zu bestehenden Symbolen wie `i-compass`. Der SEPARATE Filter-Chip „Sichtachse" ist von dieser Korrektur NICHT betroffen — er behält sein eigenes, je nach Filter-Zustand wechselndes Icon unverändert.
...
- Verifiziertes Referenzmuster (Zeilen aktualisiert nach US-21-Verschiebung, verifiziert 2026-07-06): `oppCard()` (jetzt `web/index.html:1776`) baut die bestehenden Karten-Tags zusammen; das Muster `Verify.getLast(o.location_id) ? '<span class="tag tag-verified">' + ic('i-check',12) + ' Geprüft</span>' : ''` ist weiterhin die Vorlage. Die CSS-Klassen `.tag-verified` / `.tag-issue` (unverändert Zeile 579/580) sowie weitere Vorbilder `.tag-gold`/`.tag-blue`/`.tag-warn` (jetzt Zeile 1789-1793) sind die bestehenden Farbbausteine, die wiederverwendet werden.
- Neue Pille folgt demselben Muster: ein neuer `tag-sightline`-Klassensatz (eine Basis-Klasse, Farbe via Text-Color-Modifier je Status statt vier komplett neuer Icon-Varianten) analog zu `tag-verified`/`tag-issue`, eingehängt in `oppCard()` neben den bestehenden Tags sowie im Detail-Sheet der Chance (gleiche Position wie der „Geprüft"-Tag, damit Status-Infos gebündelt an einer Stelle erscheinen).
- Icon: nur EIN neues Icon nötig (konstantes Augen-Symbol, neu in `<defs>` anzulegen, analog `i-compass`) statt der ursprünglich vorgesehenen vier verschiedenen Zustands-Icons (Häkchen/Halbdreieck/Warndreieck/Fragezeichen entfallen für die Pille ersatzlos — Farbunterscheidung trägt den Status).
- Sichtbar auf: Feed-Karte (`oppCard()`, jetzt Zeile 1776) und im Detail-Sheet der Chance (exakte Einfügeposition erst bei Implementierung festzulegen, da abhängig vom finalen Sheet-Layout).
- **Neu (2026-07-06, Konsistenz-Anforderung):** Analog zum von US-21 etablierten Hausstandard „jedes zentrale UI-Element mit eigener Bedeutung hat eine eigene, direkt abrufbare Erklärung" bekommt die neue Sichtachsen-Pille einen eigenen ⓘ-Info-Button samt Erklärtext in `ElementInfo`, analog zum Vorbild beim Verifikationsfilter (`fInfoBtn('verifikationsstatus')`, jetzt Zeile 3556, Erklärtext in `ElementInfo` Zeile 6949-6952). Das gilt gleichermaßen für den neuen Filter-Chip „Sichtachse" (Punkt 3 unten) — auch dieser bekommt einen eigenen ⓘ-Info-Button, damit die App-weite Konsistenz nicht durchbrochen wird.
...
- Betroffene, verifizierte Code-Stellen:
  ...
  - „Sichtachsen-Profil"-Box im Location-Anlegen-Preview: `AddLocation.renderPreview()` (Zeilenreferenz aktualisiert nach US-21, 2026-07-06: jetzt Z. 6609) — das ist eine statische Text-/Stat-Box (Azimut, Distanz, Höhenwinkel), **kein** SVG/Linien-Rendering. Für diese Box selbst ist kein Linienstil anpassbar (keine Linie vorhanden), nur die Astro-Kompass-Linie (Punkt oben) trägt einen sichtbaren Linienstil.
...
- Analog zum bestehenden Verifizierungsfilter (`verificationIncl[]`/`verificationExcl[]`, siehe Memory-Referenz `reference_fotoalert_verification_filter`), verifiziertes Code-Pattern in `web/index.html` (Zeilenreferenzen aktualisiert nach US-21-Verschiebung, verifiziert 2026-07-06):
  - State: `verificationIncl: [], verificationExcl: []` im Default-State (jetzt Zeile 2836); neues Analogon z.B. `sightlineIncl: [], sightlineExcl: []`.
  - Zustandslogik: `_cycleVerification` (jetzt Zeile 3581-3600, Off→Incl→Excl→Off-Zyklus, ruft `Filter.save()`) ist die 1:1-Vorlage für eine neue `_cycleSightline(v)`-Funktion.
  - Rendering: Chip-Rendering (jetzt Zeile 3406-3415) sowie die Einbindung im Filter-Sheet-HTML sind die Vorlage für die neue Sektion „Sichtachse" mit vier Chips (Frei/Teilweise verdeckt/Blockiert/Nicht geprüft).
  - Filteranwendung an den Datenquellen: der neue Sichtachsen-Filter muss analog zum bestehenden Verifikationsfilter an den entsprechenden Datenquellen-Stellen ergänzt werden (konkrete Zeilen bei Implementierung neu zu verifizieren).
  - Badge-Zähler: der neue Sichtachsen-Filter zählt nach demselben Muster wie der Verifikationsfilter in die Gesamtsumme der aktiven Filter für den Badge in der Filter-Kopfzeile mit.
  - Ausgrauen-Regel: Die bestehende BUG-46-Logik unterscheidet zwischen Location-Attributen (u.a. auch auf Karte/Locations-Tab wirksam) und Chancen-Attributen (nur Feed/Kalender/Scout wirksam), siehe Memory-Referenz `reference_fotoalert_filter_criteria`. Da der Sichtachsen-Status an der Location hängt (Precompute-Ergebnis, siehe Analyse oben), ist er fachlich näher an einem Location-Attribut als an einem Chancen-Attribut — sollte also (wie z.B. Schwierigkeitsgrad/Kategorie) NICHT auf Karte/Locations-Tab ausgegraut werden, sondern dort ebenfalls wirken. Diese Einordnung ist eine Designer-Empfehlung basierend auf der Datenherkunft, keine von Stephan explizit bestätigte Entscheidung — bei Implementierung kurz gegenprüfen.
  - **Neu (2026-07-06, Konsistenz-Anforderung, Hausstandard seit US-21):** Analog zum Verifikationsfilter-Vorbild — eigener ⓘ-Info-Button am Section-Header (`fInfoBtn('verifikationsstatus')`, jetzt Zeile 3556) mit Erklärtext in `ElementInfo` (Zeile 6949-6952) — bekommt auch der neue Filter-Chip „Sichtachse" einen eigenen ⓘ-Info-Button (z.B. `fInfoBtn('sichtachsenstatus')`) samt eigenem Erklärtext in `ElementInfo`, damit die App-weite Konsistenz „jedes zentrale UI-Element mit eigener Bedeutung hat eine eigene, direkt abrufbare Erklärung" nicht durchbrochen wird.
