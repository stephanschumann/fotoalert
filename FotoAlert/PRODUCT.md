...
| Sichtachsen-Projektion für Himmelsröte & Goldene Wolken (US-131) | Der für Himmelsröte (RED_SKY) genutzte Dunst-/Wolkenwert und der für Goldene Wolken (GOLDEN_CLOUDS) genutzte Wolkenwert werden nicht mehr am Standort des Fotografen abgefragt, sondern an einem 30 km entlang der Sichtachse projizierten Punkt — für Himmelsröte in Gegenrichtung der Sonne (Antisolarpunkt), für Goldene Wolken in Sonnenrichtung; beide getrennt berechnet (keine geteilte Wolken-Score-Berechnung mehr), identisch für den schnellen Einzelabruf-Pfad (US-106) und den regulären Cronlauf. Schlägt der Abruf am projizierten Punkt fehl, gibt es **keinen** Fallback auf den Fotografen-Standort — die betroffene Karte gilt schlicht als „Signal nicht verfügbar". Die allgemeine Wetteranzeige (Temperatur, Niederschlag, Wind, `weather_score`) bleibt davon unberührt und weiterhin am Fotografen-Standort verankert. Zusätzlich wurde die externe Wetter-API-Anbindung gedrosselt (`WEATHER_API_MAX_CONCURRENT_REQUESTS = 4` gleichzeitige Requests + `WEATHER_API_REQUEST_PACING_SECONDS = 0.25` Pacing pro Slot; ursprünglich 5/0.15, nach BUG-83 auf 4/0.25 nachjustiert), nachdem Live-Messungen ohne Drosselung ein signifikantes Rate-Limiting der externen API (bis zu 31 % HTTP 429) gezeigt hatten. Ein einzelner HTTP 429 wird seit BUG-83 zusätzlich automatisch mit steigender Wartezeit wiederholt (bis zu 2x, einheitlich für alle vier Abruf-Arten: Wetter, Aerosol, Sonnenrichtung, Gegenrichtung); andere Fehler (Timeout, sonstiger HTTP-Status, generische Exception) werden weiterhin NICHT wiederholt und bleiben ein sofort sichtbarer Fehler im Job-Status. Seit BUG-99 gibt es außerdem eine Gesamt-Zeitbegrenzung von 180 Sekunden für den kompletten Wetter-Abfrage-Durchlauf über alle Standorte hinweg — betrifft den täglichen Wetter-Vorberechnungslauf, den 3-Stunden-Cronlauf und den Einzel-Standort-Reload gleichermaßen (Konstante `WEATHER_OVERLAY_MAX_TOTAL_SECONDS = 180.0`, bemessen an ~319 aktiven Standorten: rechnerisch ca. 104 s Grundlaufzeit + 30 s Retry-Puffer + 25 % Sicherheitsmarge). Vorher konnte ein solcher Durchlauf bei Verzögerungen (z. B. Rate-Limits des Wetterdienstes) theoretisch unbegrenzt lange dauern. Wird die 180-Sekunden-Grenze erreicht, werden die bis dahin bereits erfolgreich abgerufenen Standort-Ergebnisse trotzdem verwendet — nur die zu diesem Zeitpunkt noch offenen Anfragen gelten als nicht verfügbar, der gesamte Lauf wird nicht verworfen. Im selben Zug wurde der Mindestabstand zwischen einzelnen Wetter-Anfragen von 0,25 s auf 0,35 s erhöht (`WEATHER_API_REQUEST_PACING_SECONDS = 0.35`), um Rate-Limit-Fehler von vornherein seltener zu machen. |
...
| Kartentitel mit Motiv-Bezug (BUG-94) | Alle drei Wolkenstimmungs-Kartentypen (Goldene Wolken, Himmelsröte, Rote Wolken) zeigen im Titel jetzt das Motiv statt nur den Event-Typ-Namen, Muster „{Event-Typ} über {Motiv}" (z. B. „Rote Wolken über Teufelsberg") — analog zum bestehenden Mond-Alignment-Muster. Der Event-Typ-Badge/Icon über dem Titel bleibt unverändert, die separate Standort-Zeile (Stecknadel-Icon) unter dem Titel ebenfalls. Zentraler Baustein `_build_opportunity_title()` in `backend/main.py`, verbindlich für alle Chancenart-Builder; fehlt zu einer `location_id` ein auflösbares Motiv, bleibt der Titel beim bisherigen reinen Event-Typ-Namen (kein „… über None"). Zwei automatisierte, generische Konsistenz-Wächter-Tests (`test_bug-94.py`) stellen sicher, dass auch künftig neu hinzukommende Chancenarten dieses Muster automatisch bekommen, ohne dass das manuell nachgepflegt werden muss. Gilt für Feed, Kalender und Detail-Sheet; Karte (Pin-Popup) und Scout waren nie betroffen. |
...
| Leer-State | Wenn keine Chancen passen: Hinweis-Text, kein Absturz. Reduziert ein aktiver Filter/eine Suche den kompletten 14-Tage-Feed auf 0 Treffer, aktualisiert sich seit BUG-85 auch die Kopfzeile (`#header-subtitle`) explizit auf einen eigenen, kurzen Text — „Filter: keine Treffer." bei aktivem Filter/Suche, „Keine Chancen in den nächsten 14 Tagen." ohne aktiven Filter — statt wie vorher auf dem zuletzt gültigen, ggf. veralteten Stand stehenzubleiben. Der Fall „nur 'Heute' leer, an anderen Tagen noch Treffer" bleibt davon unverändert (englische Tagline weiterhin Default). |
...
| Kalender-Mindestschwelle an Feed angeglichen (BUG-92) | Der Kalender verwendete serverseitig eine hoehere Mindest-Wahrscheinlichkeitsgrenze (0,40) als der Feed (effektiv 0,35) - Termine im Bereich 0,35-0,40 waren dadurch im Feed sichtbar, im Kalender aber nie berechnet, unabhaengig von jeder Nutzerfilterung. Grenze in allen vier Fundstellen (Live-On-Demand-Pfad in `backend/main.py`, Hintergrund-Batch-Pfad in `backend/precompute.py`) auf 0,35 angeglichen. 9 automatisierte Tests (`backend/tests/test_bug92.py`) sichern den Fall dauerhaft ab, inkl. eines gezielten Regressionstests fuer das Fallback-Vergleichsliteral. |
...
**Eventtyp — Sonderfall „Mond-Alignment" (BUG-91):**
- Der Filter-Chip „Mond-Alignment" matcht seit BUG-91 nicht mehr ausschließlich `event_type === "Mond-Alignment"`: zusätzlich eingeschlossen sind Ereignisse mit `event_type` „Vollmond"/„Supermond", deren `composition_analysis.azimuth_delta_deg` UND `altitude_delta_deg` beide innerhalb der bestehenden ±2°-Toleranz liegen (`MOON_ALIGNMENT_TOLERANCE_DEG` im Frontend, Spiegel von `backend/precompute.py::ALIGNMENT_TOLERANCE_DEG`) — die großzügige Variante, von Stephan bewusst statt einer engeren eigenen Schwelle gewählt.
- Der angezeigte Typ/Titel/Icon des Ereignisses ändert sich dabei NICHT: ein so eingeschlossenes Ereignis bleibt weiterhin als „Vollmond"/„Supermond" beschriftet, es taucht nur zusätzlich unter dem „Mond-Alignment"-Filter auf.
- Gilt für Chancen-Feed, Kalender UND Karte (zentrale Hilfsmethode `Filter._matchesExpandedType()`, von `MapView.applyFilter()` mitgenutzt statt einer zweiten eigenen Prüfung). Der Scout-Modus (`Filter.applyToScout()`) ist unverändert, da er `event_type` gar nicht nutzt (matcht auf `body_name`).
- Fehlen die Ausrichtungsdaten (`composition_analysis` = null oder eines der beiden Delta-Felder nicht numerisch), gilt das Ereignis als nicht qualifiziert und bleibt ausschließlich unter „Vollmond"/„Supermond" auffindbar. Ein schwach ausgerichtetes Vollmond-/Supermond-Ereignis (Delta außerhalb ±2°) bleibt ebenfalls ausschließlich unter „Vollmond"/„Supermond" auffindbar — der Filter wird dadurch nicht von jeder Vollmond-Nacht geflutet.
...
| Sichtachsen-Check-Auslöser | Die Sichtachsen-Prüfung läuft automatisch einmalig beim Anlegen/Ändern einer Location (Raycast über Höhendaten OpenTopoData + Gebäudedaten aus einer wöchentlich aktualisierten lokalen Datei (`building_footprints.json`, TASK-59) statt einer direkten Live-Overpass-Abfrage; bei fehlendem Cache-Treffer, z. B. neu angelegte Location, fragt die App live bei öffentlichen Overpass-Mirrors nach). Zusätzlich manuell auslösbar über den neuen Menüpunkt „Sichtachsen aktualisieren" im Location-Detail (Orte-Tab, siehe Abschnitt 7). Ergebnis wird je Location gespeichert, nicht bei jedem Feed-Aufruf neu berechnet. |
...
| Sichtachsen-Check-Pille (Location-Detail, US-09) | Zeigt denselben Status wie im Feed/Chancen-Detail (Augen-Icon + Grün „Frei" / Orange „Teilweise verdeckt" / Rot „Blockiert" / Grau „Nicht geprüft"), ⓘ-Erklärtext daneben. Ergebnis stammt aus der zuletzt gelaufenen Raycast-Prüfung (Höhendaten OpenTopoData + Gebäudedaten aus wöchentlich aktualisiertem lokalen Datei-Cache statt Live-Overpass-Abfrage, TASK-59) dieser Location. |
...
| Location bearbeiten | Edit-Modus in Location-Detail; Änderungen persistieren via PATCH + Server-Fetch. Editierbare Felder: Name, Beschreibung, Koordinaten, Brennweiten-Empfehlung, Stockwerkshöhe, **Motivname (`subject_name`)** (BUG-61), **HINWEISE (`special_notes`)** (BUG-50), **Bauwerkshöhe (`subject_height_m`)** und **Bauwerksbreite (`subject_width_m`)** (US-128, Details in eigener Zeile unten), **Kategorie** und **Schwierigkeitsgrad** (BUG-84). Kategorie- und Schwierigkeitsgrad-Feld zeigen beim Öffnen des Formulars den tatsächlich gespeicherten Wert der Location vorbelegt (nicht mehr einen festen Standardwert für alle Locations) und werden bei einer Änderung auch wirklich serverseitig übernommen (vorher wurden beide Felder von der Backend-Whitelist verworfen, trotz angezeigter Erfolgsmeldung). Das HINWEISE-Feld kann beliebig geändert oder geleert werden — der ursprüngliche Text kehrt nach dem Speichern nicht zurück. Der Motivname wird nach dem Speichern sofort in der Motiv-Sektionsüberschrift des Detail-Sheets angezeigt und bleibt auch nach komplettem App-Neuladen erhalten (BUG-61). |
...
| Playwright-pytest-Testdateien laufen real statt lautlos übersprungen (BUG-100) | Die drei Playwright-basierten pytest-Testdateien (`test_bug_85.py`, `test_bug-80.py`, `test_task-66.py`) wurden im Job `test-backend` mangels installiertem Playwright bisher lautlos als SKIPPED übersprungen — die CI blieb trotzdem grün, obwohl der eigentliche Testinhalt nie lief. Sie laufen jetzt real im ohnehin Playwright-fähigen Job `test-frontend` mit; `test-backend` deselektiert sie bewusst (Skip dort ist beabsichtigt). Zusätzlicher struktureller Guard-Test `backend/tests/test_bug100_ci_playwright_gate.py` prüft `deploy.yml` gegen einen künftigen Rückfall. |
...
| 2026-07-22 | BUG-83 | Wolkenwert-Abruf für Sonnenrichtung/Gegenrichtung schlug beim manuellen Wetter-Update für ~160 Locations mit HTTP 429 fehl, weil diese beiden Abruf-Arten pro Location bis zu 6x häufiger anfallen als der normale Wetter-Abruf (TASK-75-Drosselungsmechanismus, hier nur granularer sichtbar). Weg-Gate: Option A (Drosselung nachjustiert: `WEATHER_API_MAX_CONCURRENT_REQUESTS` 5→4, `WEATHER_API_REQUEST_PACING_SECONDS` 0.15→0.25) kombiniert mit Option B (automatischer Retry mit steigender Wartezeit bei HTTP 429 in `_run_one_weather_fetch()`, bis zu 2x, einheitlich für alle vier Abruf-Arten; andere Fehler weiterhin ohne Retry, BUG-77-Sichtbarkeit bleibt erhalten). Live-Test bestätigt: 0 Fehler, 100 % Erfolg, 127,6 s Laufzeit. Realer Release-Lauf (v1.22.42) scheiterte zunächst an 8 CI-Testfehlschlägen (Python-3.9-spezifisches `asyncio.Semaphore()`-Verhalten in `test_bug83.py`, in der eigenen Verifikation unter Python 3.11 nicht aufgetreten) — Test-Helfer korrigiert (Semaphore-Konstruktion nach Event-Loop-Start), 43/43 Tests erneut grün, keine Assertion/Testabdeckung verändert. Als einfacher Folge-Commit (kein neuer Versions-Bump, weiterhin v1.22.42) gepusht, Commit `0aff272`, CI-Lauf #254 grün (Frontend-Check Playwright, Backend-Tests pytest, Deploy FotoAlert), Health-Check bestätigt (`version 2.0.0`, `locations_count 172`) auf https://fotoalert.stephanschumann.com. |
| 2026-07-22 | TASK-86 | Vier bislang ungeschützte Endpunkte gegen Missbrauch gehärtet: neues Hilfsmodul `backend/rate_limit.py` (schlanke eigene In-Memory-Rate-Limit-Lösung, keine neue Abhängigkeit) an vier Stellen genutzt. `POST /preview-alignment` bekommt eine Häufigkeits-Bremse pro Absenderadresse (20 Aufrufe/Minute; der bereits durch BUG-63 gelöste 14-Tage-Zeitraum-Deckel bleibt unverändert). `GET /calendar`: Kalender-Zwischenspeicher-Schlüssel wird jetzt auf 2 Nachkommastellen gerundet (verhindert unnötiges Cache-Wachstum durch minimal unterschiedliche Score-Schwellen) und auf 200 Einträge mit FIFO-Verdrängung des jeweils ältesten Eintrags gedeckelt. `POST /login`: Sperre nach 5 falschen Passwörtern derselben Absenderadresse innerhalb 15 Minuten (HTTP 429 statt weiterer Passwortprüfung), Zähler setzt sich bei erfolgreichem Login zurück. `POST /register-device`: Geräte-Token wird auf Plausibilität geprüft (20–256 Zeichen, druckbares ASCII/Hex) und die Registrierungshäufigkeit pro Absenderadresse begrenzt. Alle vier Bremsen antworten einheitlich mit HTTP 429 + Klartext-Wartezeit statt stiller Verzögerung. CORS unverändert (bereits durch TASK-83 gelöst, hier nur regressionsgetestet). Nachbesserung während der unabhängigen Verifikation: `client_identity()` las zunächst den ERSTEN Eintrag eines `X-Forwarded-For`-Headers — vom Client selbst fälschbar, dadurch waren alle drei adressbasierten Bremsen umgehbar; behoben auf den LETZTEN Eintrag (in der Ein-Hop-Topologie mit Caddy als einzigem Proxy der garantiert von Caddy selbst gesetzte, nicht clientseitig fälschbare Wert), zusätzlich per dediziertem Regressionstest abgesichert. Automatisiert: `backend/tests/test_task86.py` (30/30 grün, Marker `offline`/`api`/`regression`), in `backend/tests/README.md` nachgetragen. Unabhängige Verifikation bestätigt 9/9 Akzeptanzkriterien, Refactor-Check abgeschlossen. Released als v1.22.44 (Nachbesserung v1.22.45 direkt danach: 3 CI-Regressionen behoben — Token-Testdatenlänge in test_api_regression.py, preview_alignment()-Direktaufruf-Kompatibilität in main.py), CI grün, Health-Check bestätigt (version 2.0.0, locations_count 172) auf https://fotoalert.stephanschumann.com. |
| 2026-07-27 | BUG-84 | Bearbeiten-Formular zeigte im Kategorie-Dropdown ein Feld (`category_key`), das die API nicht lieferte, und fiel deshalb immer auf denselben Standardwert zurück; beim Speichern wurde die tatsächliche Kategorie dadurch unbemerkt überschrieben (echter Datenverlust, kein reiner Anzeigefehler). Bei der Analyse ein identisches, verwandtes Problem beim Schwierigkeitsgrad gefunden und auf Stephans Entscheidung direkt mit demselben Fix-Ansatz mitbehoben statt als eigenes Ticket. Ursache tiefer als zunächst angenommen: Weg B (Anzeige + Speichern) zeigte, dass das Backend `category`/`difficulty` beim PATCH grundsätzlich nie persistierte, obwohl eine Erfolgsmeldung angezeigt wurde. Fix: `backend/data/locations.py` neue `coerce_category_value()` (robuster String→Enum-Cast mit SKYLINE-Fallback statt Absturz bei unbekannten Werten), `category`/`difficulty` in die PATCH-Feld-Whitelist aufgenommen; `backend/main.py` liefert jetzt `category_key` aus, validiert `category`/`difficulty` vor jedem Speichern (422 bei ungültigem Wert); `web/index.html` `LocationDetail.openEdit()` liest beide Felder jetzt aus den echten API-Werten. Erster Release-Versuch (v1.22.46) scheiterte in CI an einer bis dahin unentdeckten, unabhängigen Altlast in `backend/data/store.py`: `.get(key, default)` ersetzt einen Wert nur, wenn der Schlüssel ganz fehlt, nicht wenn er mit `None` vorhanden ist — bei einem frischen CI-Checkout (keine vorbefüllte Dev-Datenbank) traf das bei neu angelegten Custom-Locations zu und führte zu `TypeError: float() argument must be ... not NoneType`. In isolierter Sandbox nachgestellt (Fehler mit unverändertem Code reproduziert, mit neuer `_num_or_default()`-Hilfsfunktion an 10 Stellen behoben, 15/15 Zieltests grün, 612 Regressionstests ohne neue Fehlschläge). Zweiter Release-Versuch scheiterte zusätzlich an zwei fehlenden Zeilen in der Test-Marker-Tabelle (`backend/tests/README.md`) für zwei bereits vorhandene, aber nie eingetragene Testdateien (TASK-59, TASK-89) — per HEAD-Referenz-Abgleich sauber nachgetragen. Released als v1.22.47 + Nachbesserung Commit `bc4ab35`, CI grün (Frontend-Check Playwright, Backend-Tests pytest unter echtem Python 3.9.25, Deploy FotoAlert), Health-Check bestätigt (version 2.0.0, locations_count 172); Live-Verifikation zusätzlich per echter Bearbeiten-Aktion bestätigt (Kategorie-Vorbelegung korrekt, Schwierigkeitsgrad-Änderung gespeichert, sichtbar nach Neuladen, anschließend auf Ursprungswert zurückgesetzt) auf https://fotoalert.stephanschumann.com. Nebenbefund aus der Verifikation als eigenes Ticket (TASK-94) in die Inbox aufgenommen. |
| 2026-07-28 | BUG-85 | Kopfzeile (`#header-subtitle`, `Feed.render()`) aktualisierte sich nicht, wenn ein aktiver Filter den kompletten 14-Tage-Feed (nicht nur „Heute") auf 0 Treffer reduzierte — sie blieb auf dem zuletzt gültigen, ggf. veralteten Text stehen, weil beide `return`-Stellen im Empty-Zweig (Zeile ~2119 BUG-32-Soft-Fallback, Zeile ~2130 generischer Empty-State) vor der einzigen Kopfzeilen-Aktualisierung (Zeile 2169–2171) lagen. Ursprüngliche Ticket-Prämisse („englische Tagline ist Sprachfehler") von Stephan als falsch verworfen und auf den tatsächlichen Stale-State-Bug korrigiert. Fix (Option A, minimal-invasiv): direkt vor dem `return` im generischen Empty-Branch werden jetzt zwei neue, dedizierte, bewusst kurze Kopfzeilentexte explizit gesetzt — „Filter: keine Treffer." bei aktivem Filter/Suche, „Keine Chancen in den nächsten 14 Tagen." ohne aktiven Filter —, ohne die vorhandene Body-Leertext-Nachricht (`msg`) wiederzuverwenden (vermeidet BUG-80-Höhenrisiko). Der BUG-32-Soft-Fallback-Zweig sowie der bereits akzeptierte Fall „nur 'Heute' leer" bleiben bewusst unverändert. `backend/tests/test_bug_85.py` komplett ersetzt (alte, auf der falschen Prämisse basierende Tests entfernt). Released als v1.22.48 (Commit `29df845`) + Nachbesserung Commit `5e393a3` (CI-Fix: fehlende README-Marker-Zeile), CI grün, Live-Verifikation bestätigt (Kopfzeile zeigt korrekt „Filter: keine Treffer." bei leerem gefiltertem 14-Tage-Feed, APP_VERSION 1.22.48 live) auf https://fotoalert.stephanschumann.com. |
| 2026-07-29 | BUG-90 | Kompositions-Analyse (Höhen-/Seitenversatz in Grad und Metern) liegt jetzt auch bei Mondaufgang- und Monduntergang-Ereignissen vor: `celestial_altitude` wird im Mondaufgang-/Monduntergang-Block von `backend/calculations/opportunity.py` analog zu den 3D-Alignment-Ereignissen berechnet und gesetzt; `backend/precompute.py` (`_ALIGNMENT_FILTER_EXEMPT`) entsprechend erweitert, damit diese beiden Event-Typen von der Kompositions-Analyse nicht mehr ausgeschlossen werden. Vorher fehlte diese Geometrie-Angabe bei Mondaufgang/-untergang komplett; die Meter-Darstellung im Frontend existierte bereits, war aber mangels Daten ungenutzt. `backend/tests/test_bug-90.py` (5/5 grün), Live-Browser-Verifikation AK1–AK4 bestätigt. |
| 2026-07-29 | BUG-91 | Filter-Chip „Mond-Alignment" schließt jetzt zusätzlich Vollmond-/Supermond-Ereignisse mit sehr präziser Ausrichtung zum Motiv ein (bestehende ±2°-Toleranz `ALIGNMENT_TOLERANCE_DEG`/`composition_analysis` wiederverwendet, großzügige Variante), gilt für Feed, Kalender und Karte über die neue zentrale Hilfsmethode `Filter._matchesExpandedType()`; die betroffenen Ereignisse bleiben weiterhin korrekt als „Vollmond"/„Supermond" beschriftet, reine Frontend-Filterlogik-Erweiterung ohne Backend-/Datenmodelländerung, Scout unberührt. Released v1.22.50, CI grün, Health-Check ok, Live-Verifikation im Browser bestätigt. |
| 2026-07-30 | BUG-86 | Mondphasen-Bezeichnung korrigiert (Halbmond nur noch bei ~50% Beleuchtung statt fälschlich bei 90-97%): `_moon_phase_name()` in `backend/calculations/astronomy.py` benannte die Buckets fraction 0,28–0,47 (zunehmend) und 0,53–0,72 (abnehmend) fälschlich „Halbmond“ (tatsächlich ~59–99% Beleuchtung statt der für Halbmond erwarteten ~50%) — korrigiert auf „Zunehmender/Abnehmender Dreiviertelmond“; Bucket-Grenzen selbst unverändert (bereits lückenlos korrekt), „Halbmond“ (fachlich „Erstes/Letztes Viertel“) erscheint jetzt nur noch bei ~41–59% Beleuchtung. Released v1.22.51, Commit `8b7873f`, CI grün. Live-Verifikation nach Neustart von `fotoalert.service`: 0 von zuvor 206 betroffenen Events zeigen noch „Halbmond“ im 85–98%-Beleuchtungsbereich, alle korrekt umbenannt (Referenz-Event „Herrmannstiege“ bei 96,6% Beleuchtung: jetzt „Abnehmender Dreiviertelmond“). |
| 2026-07-31 | BUG-94 | Kartentitel bei Wolkenstimmungs-Chancen (Rote Wolken/Goldene Wolken/Himmelsröte) nennen jetzt das Motiv statt nur den Event-Typ (Muster „{Event-Typ} über {Motiv}"), zentraler Titel-Baustein `_build_opportunity_title()` + zweiter, generischer Konsistenz-Wächter-Test für das separate `opportunity.py`-Baumuster der übrigen Chancenarten (deckt künftige Chancenarten automatisch ab). Released v1.22.52 + Nachbesserung Commit `5f1ae66` (CI-Regression durch geteilte Test-Fixture-Nebenwirkung behoben, README-Marker-Lücke geschlossen), CI grün (707 passed/5 skipped/0 failed), Health-Check grün, Live-Verifikation im Browser bestätigt (~30/31 geprüfte Karten korrekt; 1 ursachlich noch ungeklärter Einzelfall als Folgeticket BUG-95 in der Inbox). |
| 2026-08-01 | BUG-92 | Kalender zeigte nicht denselben Terminbestand wie der Feed: Kalender-Mindestschwelle (0,40) war hoeher als die effektive Feed-Schwelle (0,35) - Termine dazwischen fehlten im Kalender unabhaengig von jedem Filter. Alle 4 Fundstellen (Live-On-Demand-Pfad `backend/main.py`, Hintergrund-Batch-Pfad `backend/precompute.py`) auf 0,35 angeglichen. 9 automatisierte Tests (`backend/tests/test_bug92.py`), echter Testlauf auf Stephans Mac zweimal gruen (8 passed, dann 9 passed nach Nachbesserungsrunde fuer eine von der unabhaengigen Verifikation gefundene Testluecke). Unabhaengige Verifikations- und Refactor-Phase beide durchgefuehrt, fuer Release freigegeben. |
| 2026-08-03 | TASK-59 | Sichtachsen-Check nutzt jetzt wöchentlich aktualisierte lokale Gebäudedaten-Datei statt unzuverlässiger Live-Overpass-Server; Stichprobe und Deploy-Schutz live verifiziert |
| 2026-08-03 | BUG-93 | Kalender-Vollneuberechnung nach einem `ALGORITHM_VERSION`-Bump berechnete fälschlich 0 Events statt vollständig neu: `_init_calendar_pass()` (`backend/precompute.py`) setzte `existing_meta` bei einem Versions-Mismatch lokal auf `{}` zurück, gab diesen Reset aber nicht an den Aufrufer zurück — `compute_calendar_incremental()` nutzte weiterhin seine eigene, veraltete `existing_meta`, wodurch `dates_needed = []` für jede Location resultierte. Fix (Option A): `_init_calendar_pass()` gibt `existing_meta` jetzt als 5. Rückgabewert zurück, `compute_calendar_incremental()` (einzige Aufrufstelle) verwendet diesen Wert statt seiner eigenen Kopie; Single-Location-Pfad (BUG-29) unverändert, da dort ohnehin kein Reset stattfindet. 3 automatisierte Tests (`backend/tests/test_bug-93.py`), Pflicht-Regression `test_bug29_calendar_single_recompute.py` weiterhin 7/7 grün. Zwei echte `/refresh-calendar`-Läufe auf Stephans Mac (Versions-Bump 1.4→1.5, dann zurück auf 1.4) zeigten danach 387.610 Events über ~315 Locations statt der 0, die den Bug ausmachten — von Stephan am 2026-08-03 bestätigt („passt“). Released v1.22.55, Commit `854c23f`, CI grün, `refactor_check.py` sauber. Live-Verifikation im Jahreskalender (August 2026) bestätigt: 173 echte Chancen mit Datum/Uhrzeit/Score statt der vorherigen 0. Während der Live-Verifikation zeitgleich ein unabhängiger, separater Server-Hänger beobachtet (tägliche Feed-Vorberechnung lief in ein Wetterdienst-Rate-Limit (429) und einen systemd-Timeout) — keine Überschneidung mit diesem Fix, eigenes Ticket BUG-99. |
| 2026-08-04 | BUG-99 | Der tägliche Wetter-Vorberechnungslauf, der 3-Stunden-Cronlauf und das Nachladen des Wetters für einen einzelnen Standort hatten bisher keine gemeinsame Zeitbegrenzung und konnten sich bei Verzögerungen des Wetterdienstes theoretisch unbegrenzt in die Länge ziehen; jetzt bricht der gesamte Durchlauf nach spätestens 180 Sekunden ab und nutzt die bis dahin bereits erfolgreich abgerufenen Standort-Ergebnisse weiter, statt alles zu verwerfen. Zusätzlich wurde der Abstand zwischen einzelnen Wetter-Anfragen leicht vergrößert, damit es von vornherein seltener zu Wartezeiten kommt. Released als v1.22.57, Deploy verifiziert (Health-Check ok, realer Lauf mit 142 s Laufzeit erfolgreich innerhalb der neuen 180-s-Grenze). |
| 2026-08-08 | BUG-100 | CI-Job `test-backend` installierte kein Playwright — drei pytest-Testdateien (`test_bug_85.py`, `test_bug-80.py`, `test_task-66.py`) liefen dadurch nur lautlos als SKIPPED statt real durch, CI blieb trotzdem grün. Fix (Weg-Gate Option B): Die drei Testdateien laufen jetzt zusätzlich real im ohnehin Playwright-/Server-fähigen Job `test-frontend` mit (neuer Schritt „Backend-Playwright-Tests ausführen" in `.github/workflows/deploy.yml`); `test-backend` deselektiert sie bewusst. Neuer statischer Guard-Test `backend/tests/test_bug100_ci_playwright_gate.py` prüft `deploy.yml` strukturell gegen einen künftigen Rückfall. Released v1.22.59, Commit `27f1cd3`, CI-Lauf #301 grün — im `test-frontend`-Job liefen die 3 Testdateien real (6 passed, 1 legitim/datenbedingt geskippt), im `test-backend`-Job 3 skipped wie beabsichtigt, Guard-Test 2 passed. Health-Check bestätigt auf https://fotoalert.stephanschumann.com. |
| 2026-08-09 | US-134 | Neben allen 4 Koordinaten-Eingabefeldern (Anlegen: Mein Standort/Motiv, Bearbeiten: Fotograf-Standort/Motiv) gibt es jetzt zusätzlich zum bestehenden Verlassen-des-Feldes-Schwenk (US-133) einen kompakten Bestätigen-Button, der denselben Kartenschwenk auslöst — beide Wege gleichwertig, reine Wiederverwendung der bestehenden Blur-Handler (`AddLocation._onObsBlur/_onSubjBlur`, `LocationDetail._onEditObsBlur/_onEditSubjBlur`), kein neuer JS-Code. Beim Testen echten Doppelaufruf-Bug gefunden (Blur- und Click-Ereignis feuerten beide beim schnellen Klick) und per `onmousedown="event.preventDefault()"` an allen 4 Buttons behoben, keine Auswirkung auf Tastatur-Bedienung. Released v1.22.61, Commit `2754cd3`, Deploy verifiziert (Health-Check ok, Live-Verifikation im Browser bestätigt — dabei einen bereits aktiven Service Worker auf der Produktions-URL entdeckt und bereinigt, der zunächst noch die alte Version auslieferte). |
| 2026-08-09 | TASK-89 | Caddy-Logdatei-Berechtigungsproblem aus TASK-82 dokumentiert: Fix (`deploy/setup_server.sh`, mkdir/touch/chown-Block vor Caddy-Reload) und automatisierte Tests (`backend/tests/test_task89_caddy_log_permissions.py`) lagen bereits vollständig im Repo, aus einem früheren, nie ins Ticket dokumentierten Durchlauf. Analyse dokumentiert den verifizierten Ist-Zustand nachträglich vollständig (5 AKs, AK-Qualitäts-Check bestanden, Ampel Grün). Echter Testlauf durch Stephan bestätigt: `3 passed, 4 warnings in 18.47s` (Warnungen sind vorbestehende, unabhängige FastAPI-Deprecation-Hinweise). Kein neuer Code, kein separater Release nötig — Fix war bereits committet und live.
| 2026-08-09 | TASK-90 | Beim App-Start riefen mehrere Codestellen gleichzeitig `/opportunities` ab; die Analyse konnte die ursprünglich vermuteten 3 gleichzeitigen Aufrufstellen im aktuellen Code nicht mehr nachweisen (nur 1 aktiver Aufruf beim Boot, ein zweiter erst 12s später) — Stephan entschied trotzdem für einen generellen In-Flight-Dedup-Schutz in `API.get()` (Cache-Key = vollständige URL inkl. Query-String, Map-Eintrag wird bei Erfolg UND Fehler sofort über `finally` gelöscht), der zusätzlich die deutlich größere `/locations`-Redundanz (9 Aufrufstellen) mitschützt. Unabhängig verifiziert, refaktoriert (IIFE durch `.then()`-Chain ersetzt, Konsistenz mit `CalendarView._monthCache`-Stil), live per Chrome-Browser gegen den lokalen Server getestet (AK1/AK2/AK4/AK5/AK6 sowie ein echter 12-Sekunden-Wartetest für AK3 bestätigt, gate-auditor-Urteil „Bestätigt“). Der Code wurde beim Zurücksynchronisieren aus einer Ticket-Verzeichniskopie versehentlich vom zeitgleich laufenden US-134-Release mit übernommen (`release.sh` committet immer den kompletten Dateistand), statt einen eigenen Release-Commit zu bekommen — released als Teil von v1.22.61, Commit `2754cd3`. Live auf Produktion verifiziert (`API._inflightGet` vorhanden und aktiv, `/health` → `status: ok`, `locations_count: 172`).
